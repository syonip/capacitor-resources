...
function generateForConfig(imageObj, settings, config) {
  ...
  var transformIcon = definition => {
    ...
    image
      .resize(definition.size, definition.size)
  ...
  return fs.ensureDir(platformPath).then(() => {
    ...
    var sectionName =
      "Generating " + config.type + " files for " + config.platform;
    ...
    return Q.mapSeries(definitions, def => {
      ...
      .then(() => {
        gauge.disable();
        display.success(
          "Generated " + config.type + " files for " + config.platform
        );
      })
      .catch(err => {
        gauge.disable();
...
program
  ...
  .option(
    "-i, --icon [optional]",
    "optional icon file path (default: ./resources/icon.png)"
  )
  .option(
    "-s, --splash [optional]",
    "optional splash file path (default: ./resources/splash.png)"
  )
  .option(
...
var g_settings = {
  ...
  makeicon:
    program.makeicon || (!program.makeicon && !program.makesplash)
      ? true
      : false,
  makesplash:
    program.makesplash || (!program.makeicon && !program.makesplash)
      ? true
      : false
};
...
console.log("***************************");
console.log("capacitor-resources " + pjson.version);
console.log("***************************");
